...
// Standard Chartered brand colours (Masterbrand Guidelines)
const SC_BLUE = "254498";   // primary deep blue
const SC_GREEN = "92E773";  // accent green
const SC_TINT = "D8DDED";   // ~82% lightened tint of SC_BLUE, for subcategory shading
const BODY = "262626";      // dark grey body text
...
function banner() {
  return [
    new Paragraph({
      ...
      children: [
        new TextRun({ text: "Standard Chartered Singapore Help Centre FAQ", bold: true, size: 34, color: "FFFFFF" }),
      ],
    }),
    new Paragraph({
      shading: { type: ShadingType.CLEAR, fill: SC_GREEN, color: "auto" },
      spacing: { before: 0, after: 360, line: 300 },
      alignment: AlignmentType.CENTER,
      children: [new TextRun({ text: "Section: Help", bold: true, size: 24, color: SC_BLUE })],
    }),
  ];
}
...
function category(text) {
  return new Paragraph({
    ...
    border: { bottom: { style: BorderStyle.SINGLE, size: 12, space: 4, color: SC_GREEN } },
    children: [new TextRun({ text, bold: true, size: 30, color: SC_BLUE })],
  });
}
...
function subcategory(name, url) {
  return [
    new Paragraph({
      ...
      shading: { type: ShadingType.CLEAR, fill: SC_TINT, color: "auto" },
      spacing: { before: 300, after: 0, line: 300 },
      children: [new TextRun({ text: `Help › ${name}`, bold: true, size: 26, color: SC_BLUE })],
    }),
    new Paragraph({
      shading: { type: ShadingType.CLEAR, fill: SC_TINT, color: "auto" },
      spacing: { before: 0, after: 200, line: 260 },
      children: [new TextRun({ text: `Source: ${url}`, italics: true, size: 18, color: SC_BLUE })],
    }),
  ];
}
  ...
  const out = [
    new Paragraph({
      ...
      children: [
        ...
        new TextRun({ text: q, bold: true, size: 22, color: BODY }),
      ],
    }),
  ];
    ...
    if (typeof part === "string") {
      out.push(new Paragraph({
        ...
        children: [
          ...
          new TextRun({ text: part, size: 22, color: BODY }),
        ],
      }));
    } else {
      part.bullets.forEach((b) => {
        out.push(new Paragraph({
          ...
          children: [new TextRun({ text: b, size: 22, color: BODY })],
        }));
      });
    }
  });
...
const SUBS = [
  {
    ...
    qas: [
      ...
      ["How is a registered mobile number updated before activating a Standard Chartered card?",
       ["To update a registered mobile number, log in to Online Banking or SC Mobile and go to “Update Profile Details”."]],
      ...
      ["How is a Standard Chartered card activated through Online Banking or SC Mobile?",
       ["A Standard Chartered ATM, debit or credit card can be activated and its PIN set through Online Banking or SC Mobile using these steps:",
        ...
        "SC Mobile can be downloaded free of charge from the App Store (iOS) or Google Play (Android)."]],
      ...
      ["How is overseas card usage enabled or disabled online for a Standard Chartered card?",
       ["Overseas card usage can be enabled or disabled through the Standard Chartered Singapore website or through SC Mobile, the Bank’s mobile banking app. This can only be done after the card has been activated. SC Mobile is available free of charge on the App Store (iOS) and Google Play (Android)."]],
      ...
      ["How long does overseas card usage stay enabled when it is activated by SMS on a Standard Chartered card?",
       ["Overseas card usage enabled by SMS is valid for 12 months from the date of activation. To enable overseas usage perpetually instead, replace the keyword “AOT” with “AOTP” in the SMS. An expiry date for overseas usage can be set if the function is enabled through the Standard Chartered Singapore website. Confirmation is sent by SMS once the card or cards have been successfully enabled."]],
      ...
      ["How is a card PIN set or changed using SC Mobile?",
       ["A Standard Chartered card PIN can be set or changed in SC Mobile as follows:",
  ...
  },
  {
    ...
    qas: [
      ["When should a Standard Chartered card be reported as lost or stolen?",
       ["A Standard Chartered card should be reported if it is lost or stolen, or if its PIN has been disclosed to a third person. The report can be made by logging in to Online Banking or SC Mobile."]],

      ["How is a lost or stolen Standard Chartered card reported through Online Banking or SC Mobile?",
       ["A lost or stolen Standard Chartered card is reported as follows:",
        B(["Step 1: Log in to Online Banking or SC Mobile.",
      ...
      ["How is a Standard Chartered credit or debit card PIN reset?",
       ["A Standard Chartered credit or debit/ATM card PIN is reset as follows:",
        B(["Step 1: Log in to Online Banking or SC Mobile.",
           "Step 2: Go to “Help & Services”.",
...
// ------------------------------------------------------------------ build ---
const children = [...banner(), category("Help")];
for (const sub of SUBS) {
  children.push(...subcategory(sub.name, sub.url));
  sub.qas.forEach(([q, a], i) => children.push(...qa(i + 1, q, a)));
}
...
const doc = new Document({
  ...
  styles: {
    default: { document: { run: { font: "Calibri", size: 22, color: BODY } } },
    paragraphStyles: [
      { id: "Heading1", name: "Heading 1", basedOn: "Normal", next: "Normal", quickFormat: true,
        run: { font: "Calibri", size: 30, bold: true, color: SC_BLUE } },
      { id: "Heading2", name: "Heading 2", basedOn: "Normal", next: "Normal", quickFormat: true,
        run: { font: "Calibri", size: 26, bold: true, color: SC_BLUE } },
    ],
  },
  sections: [{
